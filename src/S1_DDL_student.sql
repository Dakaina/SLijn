...
ALTER TABLE medewerkers ADD COLUMN geslacht CHAR CHECK (geslacht IN ('M', 'V'));
...
VALUES (8000, 'DONK', 'A', 'VICE D', 7839, '2002-04-14', 4000, null, 50, 'V');

UPDATE afdelingen SET hoofd = '8000' WHERE naam = 'ONDERZOEK';
...
--Begin even vanaf 70 wegens wat al gemaakte afdelingen en mislukte pogingen.
CREATE SEQUENCE seq_anr INCREMENT BY 10 START WITH 70;
ALTER TABLE afdelingen ALTER COLUMN anr TYPE numeric(3);

INSERT INTO afdelingen (anr, naam, hoofd, locatie)
VALUES(nextval('seq_anr'), 'SLAY', 7839, 'ALKMAAR'), (nextval('seq_anr'), 'KING', 7839, 'UTRECHT'), (nextval('seq_anr'), 'HQ', 7839, 'AMSTERDAM'), (nextval('seq_anr'), 'CHILLPLEK', 7839, 'ALKMAAR');
...
--postcode en telefoon met hulp van klasgenoot gemaakt omdat ik niet wist hoe ik zo een check maakte
CREATE TABLE adressen (
						  postcode char(6) CHECK (postcode ~ '^[0-9]{4}[A-Za-z]{2}$'),
						  huisnummer numeric,
						  ingangsdatum date,
						  einddatum date CHECK (einddatum > ingangsdatum),
						  telefoon char(10) UNIQUE CHECK (telefoon ~ '^[0-9]{10}$'),
...
VALUES ('1814NE', 48, '2020-04-14', '2022-04-14', '0687118555', 8000);
...
ALTER TABLE medewerkers ADD CONSTRAINT comm CHECK ((functie = 'VERKOPER' AND comm IS NOT NULL) OR (functie <> 'VERKOPER' AND comm IS NULL));
