...
FROM
    inschrijvingen
WHERE cursus = 'JAV' AND cursist IN (SELECT cursist FROM inschrijvingen WHERE cursus = 'XML');
...
FROM
    medewerkers
WHERE
    afd <> (SELECT anr FROM afdelingen WHERE naam = 'OPLEIDINGEN');
...
FROM
    medewerkers
WHERE
    mnr NOT IN (SELECT cursist FROM inschrijvingen WHERE cursus = 'JAV');
...
FROM
    uitvoeringen
WHERE date_part('year', begindatum) = 2020 AND cursus IN (SELECT code FROM cursussen WHERE type = 'BLD');
...
SELECT
    u.cursus,
    u.begindatum,
    (SELECT coalesce(count(*),0) FROM inschrijvingen i WHERE i.begindatum = u.begindatum AND i.cursus = u.cursus) as aantal_inschrijvingen
FROM
    uitvoeringen u
ORDER BY
    u.begindatum;
...
FROM
    medewerkers
WHERE
    NOT EXISTS (SELECT FROM uitvoeringen WHERE docent = mnr);
